...
//                     setPostList(res.data.data.postList.reverse());
//                 } else {
...
//                         <div>
//                             성별
//                             <span className="js-content"> {gender === "FEMALE" ? '여성' : '남성'}</span>
//                         </div>
//                     </div>
...
const CategoryPagePosting = ({ selectedCategory }) => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const res = await axios.get(`https://hyeonjo.shop/api/posts?category=${selectedCategory}&sortBy=popularity&page=0`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    }
                });
                ...
                if (res.data.isSuccess) {
                    const reversedPostList = res.data.data.posts.reverse();
